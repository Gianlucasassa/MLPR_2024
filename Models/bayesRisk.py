...
def plot_ROC_curve(llr, true_labels, output_dir):
    Pfn, Pfp, _ = compute_Pfn_Pfp_allThresholds_fast(llr, true_labels)
    plt.figure()
    plt.plot(Pfp, 1 - Pfn, label="ROC Curve")
...
def plot_bayes_error(effPriors, actDCF, minDCF, output_dir):
    effPriorLogOdds = np.log(effPriors / (1 - effPriors))
    plt.figure()
    ...
    plt.legend()
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, "bayes_error_plot.png"))
    plt.close()


def plot_confusion_matrix(confusion_matrix, class_labels, output_dir):
